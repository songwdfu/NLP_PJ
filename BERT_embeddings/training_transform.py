...
import argparse
from full_model import FullModel, WeightedCELoss, TransformModel
...
def train(model, args):
      ...
      """
      train_dataset = FNCDataset(args.train_path, output_type='ext')
      train_dataloader = DataLoader(train_dataset, batch_size=64, shuffle=True)
      val_dataset = FNCDataset(args.val_path, output_type="ext")
      ...
      weights = torch.tensor([0.02723301, 0.26196123, 0.13269961, 0.57810615]).to(device)
      # weights = torch.tensor([0.25, 0.25, 0.25, 0.25]).to(device)
      #######################
      ...
      for e in range(1, epochs+1):
            ...
            for batch in tqdm(train_dataloader, total=len(train_dataloader)):
                  opt.zero_grad()
                  head_trans, body_trans, external, labels = batch  # head_ext, body_ext
                  # input = torch.cat([head_trans, body_trans, external], dim=1).to(device)
                  labels = labels.to(device)
                  output = model(head_trans.to(device), body_trans.to(device), external.to(device))
            ...
            for batch in val_dataloader:
                  head_trans, body_trans, external, labels = batch
                  # input = torch.cat([head_trans, body_trans, external], dim=1).to(device)
                  labels = labels.to(device)
                  with torch.no_grad():
                        output = model(head_trans.to(device), body_trans.to(device), external.to(device))
            ...
            if no_improvement >= patience:
                  ...
                  print(rep)
                  torch.save(model.state_dict(), f'checkpoints_mpnet/total_ext_model_ep{e}.pth')
                  break
...
if __name__ == '__main__':
      ...
      parser.add_argument('--train_path', type=str, default='/sbert_embeddings/total_train_mean_ext.pkl')
      parser.add_argument('--val_path', type=str, default='/sbert_embeddings/total_train_mean_ext.pkl')
      parser.add_argument('--save_intvl', type=int, default=10)
      args = parser.parse_args()
      
      device = torch.device(f'cuda:{args.device}' if torch.cuda.is_available() else 'cpu')

      # input_shape = 768*2 + 22
      input_shape = 768

      model = TransformModel(input_shape)
      
      history = train(model, args)
      
      pkl.dump(history, open(f'checkpoints_mpnet/total_ext_fnc_history_ep{args.epochs}.pkl','wb'))
