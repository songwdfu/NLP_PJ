...
def train(model, args):
      ...
      """
      train_dataset = FNCDataset(args.train_path, output_type='ext_only')
      train_dataloader = DataLoader(train_dataset, batch_size=64, shuffle=True)
      val_dataset = FNCDataset(args.val_path, output_type="ext_only")
      ...
      for e in range(1, epochs+1):
            ...
            for batch in tqdm(train_dataloader, total=len(train_dataloader)):
                  opt.zero_grad()
                  external, labels = batch  # head_ext, body_ext
                  input = torch.tensor(external).to(device)
            ...
            if no_improvement >= patience:
                  ...
                  print(rep)
                  torch.save(model.state_dict(), f'checkpoints_mpnet/total_ext_only_model_ep{e}.pth')
                  break
...
if __name__ == '__main__':
      ...
      parser.add_argument('--val_path', type=str, default='/sbert_embeddings/total_val_mean_ext.pkl')
      ...
      pkl.dump(history, open(f'checkpoints_mpnet/total_ext_only_history_ep{args.epochs}.pkl','wb'))
